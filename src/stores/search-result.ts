...
export const useSearchResultStore = defineStore('search_result', {
  state: () => ({
    apiUrl: '/search_fts',
    pk: <string>'',
    list: [] as SearchResult[],
    detail: {} as SearchResult,
  ...
  getters: {
    find: (state) => (pk: string) => {
      ...
      if (pk in state.byPk) {
        return state.byPk[pk as keyof typeof state.byPk] as SearchResult
      } else {
        return {}
      }
    },
  ...
  actions: {
    ...
    setAllByPk() {
      const byPk: object = {}
      const allPks: string[] = []
      this.list.forEach((item: SearchResult) => {
